...
require 'predictive_vanilla_atari3'

if not dqn then
    require 'initenv'
end

local nql = torch.class('dqn.NeuralQPredictiveLearner_vanilla')
...
function nql:__init(args)
    ...
    p_scheduler_iteration = 0--torch.zeros(1)
    ...
    self.p_enc:share(self.enc, 'weight', 'bias', 'gradWeight', 'gradBias')
...
function nql:qLearnEnvironment()
    ...
    local pred_batch_size = self.minibatch_size*2--`global_args.learn_freq
    ...
    rmsprop(feval_encoder, s, self.enc_w, self.enc_optim_state)
    rmsprop(feval_decoder, s, self.p_dec_w, self.dec_optim_state)
    ...
    -- print('exp'..p_schedule_weight_exp)
...
function nql:feval(input)
    ...
    local loss = self.pred_criterion:forward(output, input)
    local grad_output = self.pred_criterion:backward(output, input):clone()
