...
-- partial forward (no clearState because you can't call it on a container)
-- table of inputs
...
function minimal:net1forward()
    ...
    collectgarbage()
    local mid1clone = self.net1clone.modules[1]:forward(input1)
    local out1clone = self.net1clone.modules[2]:forward(mid1clone:clone())

    collectgarbage()
    local mid1 = self.net1.modules[1]:forward(input1)
    local out1 = self.net1.modules[2]:forward(mid1:clone())
end
...
function minimal:feval(params,funcinput2)
    ...
    local funcinput2 = {torch.rand(3,2),torch.rand(3,2)}

    -- Forward on net2
    out2 = self.net2:forward(funcinput2)
    loss = self.crit:forward(out2,funcinput2[2])

    -- Backward on net2
    local gout2 = self.crit:backward(out2, funcinput2[2]):clone()
    self.net2:backward(funcinput2,gout2)
    ...
    self.enc_dw:clamp(-5, 5)
    self.dec2_dw:clamp(-5, 5)

    print('After clamp: These should be equal')
    print(self.enc_dw:sum()..'\tenc_dw:sum()')
    print(self.enc2.modules[1].gradWeight:sum()+
            self.enc2.modules[1].gradBias:sum()..
            '\tnet2.modules[1].modules[1].gradWeight:sum()')
